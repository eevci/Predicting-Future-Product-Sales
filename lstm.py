...
def prepareLSTMModel(learning_rate, input_shape, hidden_layer, lr_decay, dropout_rate):
    model = Sequential()
    model.add(LSTM(hidden_layer, input_shape=(input_shape[1], input_shape[2])))
...
def prepareLSTMModel2(learning_rate, input_shape, hidden_layer, lr_decay, dropout_rate):
    model = Sequential()
    model.add(LSTM(hidden_layer, input_shape=(input_shape[1], input_shape[2]), return_sequences=True))
    model.add(Dropout(dropout_rate))
    model.add(LSTM(int(hidden_layer / 2), input_shape=(input_shape[1], input_shape[2])))
    model.add(Dropout(dropout_rate))
    model.add(Dense(30))
    model.compile(loss='mean_squared_error', optimizer=tf.keras.optimizers.Adam(lr=learning_rate, decay=lr_decay))
    print(model.summary())
...
def prepareLSTMModel3(learning_rate, input_shape, hidden_layer, lr_decay, dropout_rate):
    model = Sequential()
    model.add(LSTM(hidden_layer, input_shape=(input_shape[1], input_shape[2])))
    model.add(Dropout(dropout_rate))
...
def trainWithContinousData():
    ...
    def start(modelFunc, learning_rate, hidden_layer, lr_decay, dropout_rate, show_plot, verbose):
        ...
        figure_name = 'lstm-model%d-%d-lr%f-drop%f-lrdecay%f-epoch%d-rmse%f' % (
        type, hidden_layer, learning_rate, dropout_rate, lr_decay, epoch, rmse)
        ...
        for hid_layer in hidden_layer:
            for lr_dcy in lr_decay:
                for dropout in dropout_rate:
                    for modelFunc in [prepareLSTMModel]:
                        try:
...
def predictTest():
    ...
    print(test_x_set.shape)
    print(test_y_set.shape)
    model, type = prepareLSTMModel(0.001, test_x_set.shape, 256, 0.0001, 0.5)
    model.load_weights('./models/lstm-best.h5')
    y_pre =model.predict(test_x_set)
    y_pre = y_pre[:, -1]
    plotGraphs(test_y_set[:, -1], y_pre, None, "")

    rmse = np.sqrt(((y_pre - test_y_set[:, -1]) ** 2).mean())
    print(rmse)
predictTest()