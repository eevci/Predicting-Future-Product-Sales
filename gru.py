...
from tensorflow.python.keras._impl.keras.layers import SimpleRNN, RNN, GRU
from tensorflow.python.keras.layers import LSTM, Dense, Dropout

from dataset.data_utils import *


def prepareGRUModel(learning_rate, input_shape, hidden_layer, lr_decay, dropout_rate):
    model = Sequential()
    model.add(GRU(hidden_layer, input_shape=(input_shape[1], input_shape[2])))
...
def prepareGRUModel2(learning_rate, input_shape, hidden_layer, lr_decay, dropout_rate):
    model = Sequential()
    model.add(GRU(hidden_layer, input_shape=(input_shape[1], input_shape[2]), return_sequences=True))
    model.add(Dropout(dropout_rate))
    model.add(GRU(hidden_layer, input_shape=(input_shape[1], input_shape[2])))
    model.add(Dropout(dropout_rate))
    model.add(Dense(30))
...
def trainWithContinousData():
    ...
    def start1(modelFunc, learning_rate, hidden_layer, lr_decay, dropout_rate, show_plot, verbose):
        ...
        figure_name = 'gru-model%d-%d-lr%f-drop%f-lrdecay%f-epoch%d-rmse%f' % (type, hidden_layer, learning_rate, dropout_rate, lr_decay, epoch, rmse)
        model.save_weights("./models/model-" + figure_name + ".h5")
        if (show_plot == True):
            plotGraphs(test_y_set[:, -1], y_pre, history, figure_name)
        ...
        for hid_layer in hidden_layer:
            for lr_dcy in lr_decay:
                for dropout in dropout_rate:
                    for modelFunc in [prepareGRUModel, prepareGRUModel2]:
                        try:
                            print(start1(modelFunc, learning_rate, hid_layer, lr_dcy, dropout, True, 1))
...
def predictTest():
    ...
    print(test_x_set.shape)
    print(test_y_set.shape)
    model, type = prepareGRUModel2(0.001, test_x_set.shape, 256, 0.001, 0.5)
    model.load_weights('./models/gru-best.h5')
    y_pre =model.predict(test_x_set)
    y_pre = y_pre[:, -1]
